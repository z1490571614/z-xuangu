...
def get_next_trading_day(
    ...
    for i in range(1, 30):
        ...
        # 限制最大日期不超过当前日期+30天
        if next_date > date.today() + timedelta(days=30):
            break
        if is_trading_day(next_date, trading_calendar):
...
def get_trading_days_in_range(
    ...
    while current <= end:
        if is_trading_day(current, trading_calendar):
...
def get_trading_day_offset(
    ...
    max_iterations = abs(offset) * 3  # 最多检查3倍天数
    
    for _ in range(max_iterations):
        current_date += timedelta(days=direction)
        
        # 日期范围限制
        if offset > 0 and current_date > date.today() + timedelta(days=365):
            logger.warning(f"日期偏移超出范围，返回当前计算结果")
            break
        if offset < 0 and current_date < date(2000, 1, 1):
            logger.warning(f"日期偏移超出范围，返回当前计算结果")
            break
