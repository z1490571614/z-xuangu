...
def test_list_models_groups_versions_and_marks_active_available(db, tmp_path):
    ...
    db.commit()
    model_path = tmp_path / "active_auction.pkl"
    ...
    model = result["models"]["active_auction_lgbm"]
...
def test_activate_model_version_deactivates_previous_active(db, tmp_path):
    ...
    db.add_all([
        ModelVersion(model_name="active_auction_lgbm", version="old", model_path=str(old_path), is_active=1),
        ModelVersion(model_name="active_auction_lgbm", version="new", model_path=str(new_path), is_active=0),
    ])
    db.commit()

    result = model_management_service.activate_model_version(db, "active_auction_lgbm", "new")

    assert result["active_version"] == "new"
    assert db.query(ModelVersion).filter_by(model_name="active_auction_lgbm", version="old").one().is_active == 0
    assert db.query(ModelVersion).filter_by(model_name="active_auction_lgbm", version="new").one().is_active == 1
...
def test_refresh_record_predictions_updates_active_auction_fields(db, monkeypatch, tmp_path):
    Base.metadata.create_all(bind=engine)
    ...
    result = model_management_service.refresh_record_predictions(db, "active_auction_lgbm", 46)

    db.refresh(stock)
    assert result["updated_count"] == 1
    assert float(stock.model_score) == 67.89
    assert stock.model_version == "v1"
...
def test_models_api_returns_model_center_payload(client, db, tmp_path):
    ...
    data = resp.json()["data"]
    assert data["models"]["active_auction_lgbm"]["active_version"]["version"] == "v1"
